...
const HospitalDetails = ()=>{
  const hosDetails = useLocation().state;
  const {service_organization,workingHours_organization} = hosDetails
    ...
    <div>
      ...
      <section class="about-4" id="about">
        ...
        <Grid container spacing={2} className='container' sx={{mt:2}}>
          ...
          <h1>{hosDetails.organizationName_organization}</h1>
          <div>{hosDetails.address_organization},{hosDetails.city_organization},{hosDetails.state_organization},{hosDetails.country_organization}</div>
          </Grid>
          <Grid item xs={12} md={6} lg={6} sx={{mt:2}} className='clinicgrid'>
            <h1>About Clinic</h1>
            <p>
            {hosDetails.about_organization}
            </p>
            <div>
            <h3>Working Hours</h3>
            <ul>
          ...
          <ul>
            {services.Service && services.Service.map((item,index)=>(
             <li key={index}><span>{item.services}</span></li>
            ))}
          </ul>
           </div>
          </Grid>
